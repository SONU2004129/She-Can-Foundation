    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setLoginError(
                    data.error ||
                        "Incorrect Password. Check .env.example or settings.",
                );
            }
        } catch (err) {
        ...
        return (
            <div className="mx-auto max-w-md px-4 py-24 text-center">
                <div className="rounded-3xl border border-nat-border bg-white p-8 shadow-xl shadow-nat-sage/5">
                    ...
                    <h2 className="font-serif text-2xl font-bold text-nat-heading">
                        Admin Database Portal
                    </h2>
                    <p className="mt-2 font-sans text-xs text-nat-desc leading-relaxed">
                        Please enter your administrator password to authenticate
                        direct access to submissions data, database status, and
                        admin logs.
                    </p>
                    ...
                    >
                        <div>
                            <label
                                htmlFor="adminKey"
                                className="block text-xs font-bold uppercase tracking-widest text-nat-muted"
                            >
                                Admin Password
                            </label>
                            <input
                    ...
                    <p className="mt-6 font-sans text-[11px] text-nat-muted">
                        Forgot password? Set{" "}
                        <code className="rounded bg-nat-light-card px-1 py-0.5 text-[10px] text-nat-desc">
                            ADMIN_PASSWORD
                        </code>{" "}
                        in your secrets.
                    </p>
                ...
                <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
                    <div className="space-y-1">
                        ...
                        <p className="font-sans text-xs text-[#8E877F]">
                            Direct connection schema to form responses. This
                            provides everything required for your internship
                            submission.
                        </p>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        <div className="flex items-center gap-2 rounded-xl bg-white/5 px-3 py-1.5 font-sans text-xs font-medium text-white">
                            ...
                            <span>Status:</span>
                            <span
                                className={`inline-block h-2 w-2 rounded-full ${dbStatus?.connected ? "bg-nat-sage animate-pulse" : "bg-[#D99175] animate-pulse"}`}
                            ></span>
                            <span className="font-bold text-white">
                                {dbStatus?.type || "Local Storage"}
                            </span>
                        </div>
                ...
                {!dbStatus?.mongoUriConfigured && (
                    <div className="mt-4 border-t border-[#3D3A36] pt-4 flex flex-col md:flex-row md:items-center gap-3">
                        ...
                        <div className="text-xs text-[#8E877F] leading-normal flex-1">
                            ...
                            Currently saving submissions to local file{" "}
                            <code className="bg-[#1C1A18] px-1 py-0.5 rounded text-nat-terracotta font-mono text-[10px]">
                                data/submissions.json
                            </code>
                            . To connect your database, add{" "}
                            <strong className="text-white">MONGODB_URI</strong>{" "}
                            in the AI Studio Settings secrets panel. The backend
                            automatically switches with zero-downtime!
                        </div>
                    </div>
                )}
            </div>
                ...
                {[
                    {
                        ...
                        value: stats.total,
                        color: "border-nat-border bg-white text-nat-heading",
                    },
                    {
                        label: "New Forms",
                        value: stats.new,
                        color: "border-nat-border bg-white/70 text-nat-terracotta",
                    },
                    {
                        label: "Read Checks",
                        value: stats.read,
                        color: "border-nat-border bg-white/70 text-nat-sage",
                    },
                    {
                        label: "Contacted/Replied",
                        value: stats.replied,
                        color: "border-nat-border bg-nat-light-card text-nat-heading",
                    },
                ].map((card, i) => (
                    <div
                        key={i}
                        className={`rounded-xl border p-4 shadow-sm md:p-6 transition-all ${card.color}`}
                    >
                        <div className="text-[10px] font-bold uppercase tracking-widest text-[#857E78]">
                            {card.label}
                        </div>
                        <div className="mt-1.5 font-serif text-2xl font-bold sm:text-3xl">
                            {card.value}
                        </div>
                    </div>
                ))}
                ...
                <div className="lg:col-span-12 xl:col-span-7 bg-white border border-nat-border rounded-3xl p-5 shadow-sm space-y-4">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <h3 className="font-serif font-bold text-nat-heading text-lg flex items-center gap-2">
                            ...
                            <span className="text-xs font-semibold text-nat-muted">
                        ...
                        <div className="flex flex-wrap gap-1 bg-nat-light-card p-1 rounded-xl">
                            {(["all", "new", "read", "replied"] as const).map(
                                (stat) => (
                                    <button
                                        key={stat}
                        ...
                        {filteredSubmissions.length === 0 ? (
                            ...
                            <div className="divide-y divide-nat-border max-h-[500px] overflow-y-auto">
                                {filteredSubmissions.map((sub) => {
                                    ...
                                    let badgeColors =
                                        "bg-nat-light-card text-nat-muted";
                                    if (sub.status === "new")
                                        badgeColors =
                                            "bg-[#FAF5F2] text-nat-terracotta border border-nat-terracotta/20";
                                    if (sub.status === "read")
                                        badgeColors =
                                            "bg-[#F1F5F1] text-nat-sage border border-nat-sage/20";
                                    if (sub.status === "replied")
                                        badgeColors =
                                            "bg-nat-heading text-white";
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <span
                                                className={`block shrink-0 h-2.5 w-2.5 rounded-full mt-1.5 ${
                                                    sub.status === "new"
                                                        ? "bg-nat-terracotta animate-pulse"
                                                        : sub.status === "read"
                                                          ? "bg-nat-sage"
                                                          : "bg-nat-heading"
                                                }`}
                                            ...
                                            <div className="flex-1 space-y-1 min-w-0">
                                                <div className="flex items-center justify-between gap-2">
                                                    <h4 className="font-sans font-semibold text-nat-heading text-sm truncate">
                                                        {sub.name}
                                                    </h4>
                                                ...
                                                <p className="font-sans text-xs text-nat-desc truncate">
                                                    {sub.email}
                                                </p>
                                                <p className="font-sans text-xs text-nat-muted line-clamp-1 italic">
                                                    "{sub.message}"
                                                </p>
                                            </div>
                                            ...
                                            <div className="flex items-center gap-2">
                                                <span
                                                    className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded ${badgeColors}`}
                                                >
                                                    {sub.status}
                                                </span>
                ...
                <div className="lg:col-span-12 xl:col-span-5 relative">
                    {selectedSubmission ? (
                        <div className="bg-white border border-nat-border rounded-3xl p-6 shadow-md sticky top-20 text-left space-y-6">
                            <div className="flex items-start justify-between gap-4 pb-4 border-b border-nat-border">
                                <div>
                                    ...
                                    <h3 className="font-serif font-bold text-nat-heading text-lg mt-1">
                            ...
                            <div className="space-y-3 pt-2 text-xs">
                                <div className="flex items-center gap-2.5 text-nat-desc">
                                    ...
                                    <a
                                        ...
                                        className="hover:underline font-semibold text-nat-terracotta font-sans"
                                    >
                            ...
                            <div className="space-y-2">
                                <div className="text-nat-heading font-bold text-xs uppercase tracking-widest">
                                    Update State
                                </div>
                                    ...
                                    {[
                                        {
                                            ...
                                            label: "Mark New",
                                            activeColor:
                                                "bg-nat-terracotta text-white",
                                        },
                                        {
                                            status: "read" as const,
                                            label: "Mark Read",
                                            activeColor:
                                                "bg-nat-sage text-white",
                                        },
                                        {
                            ...
                            <div className="space-y-2.5 pt-2">
                                <div className="flex items-center justify-between">
                                    <label
                                        htmlFor="notes"
                                        className="text-nat-heading font-bold text-xs uppercase tracking-widest"
                                    >
                        ...
                        <div className="rounded-3xl border border-dashed border-nat-border p-12 text-center text-nat-muted sticky top-20 bg-nat-light-card/40">
                            ...
                            <h3 className="font-serif text-sm font-bold text-nat-heading">
                                No Submission Selected
                            </h3>
                            <p className="mt-1 font-sans text-[11px] text-nat-muted leading-normal max-w-[200px] mx-auto">
                                Select an item from the list on the left to
                                inspect detailed text contents, log notes, or
