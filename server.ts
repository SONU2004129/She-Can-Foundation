...
if (MONGO_URI) {
    try {
        ...
        console.warn(
            "[DB] Failed parsing custom database name from MONGODB_URI",
            err,
        );
    }
}
...
async function getMongoDBCollection() {
    ...
    try {
        ...
        console.error(
            "[DB] MongoDB connection failed. Falling back to local JSON database.",
            err.message,
        );
...
async function updateSubmissionItem(
    id: string,
...
app.post("/api/submissions", async (req, res) => {
    try {
        ...
        console.log(
            `[Form Submit] Successfully received submission from: ${newItem.email}`,
        );
        res.status(201).json({
            success: true,
        ...
        res.status(500).json({
            error: "Internal server error while processing form.",
        });
    }
});
...
app.patch("/api/admin/submissions/:id", runAdminAuth, async (req, res) => {
    try {
        ...
        if (updated) {
            res.json({
                success: true,
                message: "Submission updated updated successfully.",
            });
        } else {
...
app.delete("/api/admin/submissions/:id", runAdminAuth, async (req, res) => {
    try {
        ...
        if (deleted) {
            res.json({
                success: true,
                message: "Submissions item deleted successfully.",
            });
        } else {
            res.status(404).json({ error: "Submission item not found." });
        }
...
async function startServer() {
    ...
    app.listen(PORT, "0.0.0.0", () => {
        console.log(
            `[Server] She Can Foundation server running on port ${PORT}`,
        );
        console.log(
